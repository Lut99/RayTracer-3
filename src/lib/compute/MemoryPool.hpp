...
namespace RayTracer::Compute {
    ...
    private:
        ...
        Buffer(VkBuffer buffer, VkDeviceSize buffer_size);
    ...
    private:
        ...
        inline static Buffer init_buffer(const UsedBlock& used_block) { return Buffer(used_block.vk_buffer, used_block.length); }
    ...
    public:
        ...
        MemoryPool(const GPU& gpu, uint32_t memory_type, VkDeviceSize n_bytes);
        ...
        inline Buffer operator[](BufferHandle buffer) const { return init_buffer(this->vk_used_blocks.at(buffer)); }
        ...
        static uint32_t select_memory_type(const GPU& gpu, VkMemoryPropertyFlags mem_properties, VkBufferUsageFlags usage_flags, VkSharingMode sharing_mode = VK_SHARING_MODE_EXCLUSIVE, VkBufferCreateFlags create_flags = 0);
