...
namespace RayTracer {
    ...
    private:
        /* Reference to the GPU where this frame is rendered to. */
    ...
    public:
        ...
        void add_binding(Compute::DescriptorSetLayout& descriptor_set_layout);
        /* Given a DescriptorSet, writes the internal buffer to it. The given index is the binding created using add_binding(). */
        void set_descriptor(Compute::DescriptorSet& descriptor_set, uint32_t binding_index);
