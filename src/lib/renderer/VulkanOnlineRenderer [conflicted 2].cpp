...
void VulkanOnlineRenderer::render(Camera& cam) const {
    ...
    while (!glfwWindowShouldClose(glfw_window)) {
        ...
        vkWaitForFences(*this->gpu, 1, &image_in_flight_fences[current_frame], VK_TRUE, UINT64_MAX);
        vkResetFences(*this->gpu, 1, &image_in_flight_fences[current_frame]);
        ...
        if ((vk_result = vkQueueSubmit(gpu->compute_queue(), 1, &submit_info, image_in_flight_fences[current_frame])) != VK_SUCCESS) {
        ...
        populate_present_info(present_info, *swapchain, swapchain_index);
